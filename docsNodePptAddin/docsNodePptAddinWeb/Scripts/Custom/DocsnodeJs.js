...
const GET_TEMPLATE_PREVIEW_IMAGE = "https://docsnode-functions.azurewebsites.net/api/GetTemplatePreviewImage?code=rSXo8K/pQEEwI9XogGKhT0HmWTunxBHKlKDQj0tBi3ss8Fw4Y6/4Ow==";
const GET_MY_FOLLOWED_SITES = "https://docsnode-functions.azurewebsites.net/api/GetMyFollowedSites?code=SoU2EV68620ipQpH3SyzTY0vZldKCEYCzIOo8AfBB4IG2AFTahOZTw==";
...
$(document).ready(function () {
    ...
    //Hide slide section in both platform, as VSTO Add-in is being used for insert slide.
    $("#liSlide").hide();
    $('#btnRefresh').hide();
...
BRDocsNodeJS.load = function () {
    ...
    var docsNodePPTExtention = ".ppt";
    var docsNodePPTXExtention = ".pptx";
    var docsNodePOTExtention = ".pot";
    var docsNodePOTXExtention = ".potx";
    var fileExtensionsArray = [".ppt", ".pptx", ".pot", ".potx"];
    var docsNodeListTemplateLogo = "ppt-logo.png";
        ...
        $.ajax({
            ...
            for (var i = 0; i < result.d.results.length; i++) {
                if (result.d.results[i].ConfigAssestTitle == "Template Library") {
                    ...
                    localStorage.setItem('TemplateLibraryListGUID', result.d.results[i].ConfigSourceListGUID);
                    localStorage.setItem('TemplateLibraryList', result.d.results[i].ConfigSourceList);
                    getallViews();
                }
                else if (result.d.results[i].ConfigAssestTitle == "Images Library") {
                    localStorage.setItem('ImageLibraryListGUID', result.d.results[i].ConfigSourceListGUID);
                    localStorage.setItem('ImageLibraryList', result.d.results[i].ConfigSourceList);
                    localStorage.setItem('ImageSourceListPath', result.d.results[i].ConfigSourceListPath);
                }
                else if (result.d.results[i].ConfigAssestTitle == "Category List") {
                    localStorage.setItem('CategoryListGUID', result.d.results[i].ConfigSourceListGUID);
                    localStorage.setItem('CategoryList', result.d.results[i].ConfigSourceList);
                }
                else {
                    localStorage.setItem('SlidesLibraryListGUID', result.d.results[i].ConfigSourceListGUID);
                    localStorage.setItem('SlidesLibraryList', result.d.results[i].ConfigSourceList);
                }
            }
        ...
        try {
            ...
            if (docName == "" || docName == null || docName.match('%') || docName.match('"') ||
                ...
                getLocalForageItem("CurrentTemplateDir").done(function (values) {
                    ...
                    var docnameLen = docName.length;
                    var newFileName = docName + docsNodePPTXExtention;
        ...
        try {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    favdef.resolve(JSON.parse(data));
                },
                error: function (data) {
        ...
        if (SPToken && !items) {
            ...
            $.ajax({
                ...
                if (!response.folderAccess) {
                    var Files = response.d.Files.results.filter(data =>
                        (data.Name.lastIndexOf(".ppt") > -1 ||
                            data.Name.lastIndexOf(".pot") > -1)
                    );
            ...
            if (!items.folderAccess) {
                var Files = items.d.Files.results.filter(data =>
                    (data.Name.lastIndexOf(".pot") > -1 || data.Name.lastIndexOf(".ppt") > -1)
                );
        ...
        try {
            ...
            getListItemsForView(selectedViewName).done(function (data) {
                ...
                if (items.length != 0) {
                    ...
                    filterstr += ") and (substringof('" + docsNodePPTExtention + "', FileLeafRef))";
                    filterstr += ")";
        ...
        try {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (data[i].ContentType.Name === "Document") {
                    ...
                    pptString += "<i class='ms-Icon ms-Icon--PowerPointLogo' title='PowerPointLogo' aria-hidden='true'></i>";
                } else {
    ...
    function getImageCORSCall(boxThumbnailURL, cnt) {
        ...
        try {
            ...
            let payload = {
                tenant: ORG_TENANT.id,
                SPOUrl: ORG_ROOT_WEB.webUrl,
                TemplateServerRelURL: templateServerRelURL,
                FileServerRelativeUrl: boxThumbnailURL
            };

            $.ajax({
                url: GET_TEMPLATE_PREVIEW_IMAGE,
                type: "POST",
                contentType: "application/json",
                data: JSON.stringify(payload),
                headers: {
        ...
        if (data.length > 0) {
            data.map(function (item) {
                ...
                if (item.ContentType.Name === "Document") {
                    ...
                    //boxThumbnailURL += "DocsNodeAdmin/_layouts/15/getpreview.ashx?path=" + fileURL;
                    var fileReletivePath = fileURL.indexOf(ORG_ROOT_WEB.webUrl) >= 0 ? fileURL.substr(fileURL.indexOf(ORG_ROOT_WEB.webUrl) + ORG_ROOT_WEB.webUrl.length) : fileURL;

                    getImageCORSCall(fileReletivePath, Cnt).done((respItem) => {
    ...
    function bindingWebAllTemp(imageURLs, data, i) {
        ...
        fileExt = fileExt.toLowerCase();
        if (
            data[i].Name.toLowerCase().indexOf(docsNodePPTExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodePPTXExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodePOTExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodePOTXExtention) != -1 ||
            data[i].ContentType.Name === "Folder") {
    ...
    function binding(imgdata, data, i) {
        ...
        if (data[i].Name.toLowerCase().indexOf(docsNodePPTExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodePPTXExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodePOTExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodePOTXExtention) != -1 ||
            data[i].ContentType.Name === "Folder") {
        ...
        try {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    dfdDocPreview.resolve(data);
                },
                error: function (data) {

                    dfdDocPreview.reject(data);
                }
            });
    ...
    // Loader
    function openWaitDialog(pagenumber) {
        $searchResultsDiv.html("");
        if (pagenumber == "first") {
            $("#WaitDialogFirst").show();
        }
        else {
            $("#WaitDialog").show();
        }
